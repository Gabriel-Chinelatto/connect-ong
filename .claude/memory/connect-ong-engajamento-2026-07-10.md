...
## 2ª RODADA DE FEEDBACK (apos ver ao vivo de novo) — 2 correcoes
1. **Overflow de pixels no painel da ONG:** (a) o item de menu da AppBar com rotulo longo ("Perfil da ONG (capa, endereço, fotos)") estourava -> `_menuItem` agora poe o Text em `Flexible`+ellipsis e o rotulo virou "Editar perfil da ONG"; (b) as sub-abas de Interesses trocaram o `SegmentedButton` (nao quebra linha, estoura em tela estreita) por CHIPS (`ChoiceChip` num `Wrap`, helper `_chipInteresse`) que quebram linha. LICAO: em Flutter, itens de PopupMenu com Row+Text longo precisam de Flexible; e SegmentedButton com 3+ segmentos estoura em largura apertada — Wrap de chips e mais seguro.
2. **Marcar UMA notificacao como lida (nos 2 apps):** o backend JA tinha `PUT /notificacoes/{id}/lida` (com checagem de dono no service) — so faltava a UI. Agora tocar numa notificacao marca so ela (otimista: `models/notificacao` ganhou `copyWith(lida:)`, service ganhou `marcarLida(id)`; a tela atualiza o item local + chama o backend). Mantido o "Marcar todas" e, no mobile, a navegacao de PRESTACAO -> Matches/Concluidas.

## ESTADO FINAL (fim da sessao 2026-07-10)
Tudo commitado e no GitHub (3 repos). Backend 141 testes verdes; `flutter analyze` limpo nos 2 apps. Verificado AO VIVO no MySQL 5.6 real da escola (as 3 migracoes desta sessao — avaliacao_doador_foto, data_status — rodaram no MySQL real; risco de feira zerado). Pendencias humanas continuam (ver [[connect-ong-auditoria-final]]): rotacionar senha do MySQL no git; `APP_DEMO_ENABLED=true` na maquina da feira. Nenhuma tarefa de codigo aberta desta leva.
