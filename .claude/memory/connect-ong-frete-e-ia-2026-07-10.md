...
- **Bug de salvar o "Sobre"** ("tamanho deve ser entre 0 e 255"): a descrição da ONG era VARCHAR(255). Ampliada para **1000** (entidade `@Size`/`@Column`, `OngUpdateDTO`, + changeset Liquibase no `db.changelog-master.yaml`). O "Sobre" da IA é capado em ~600. **GOTCHA importante**: o 1º changeset (`ong-descricao-varchar-1000`) usava precondição `columnExists` com `onError/onFail: MARK_RAN` e foi **MARK_RAN sem rodar o ALTER** → a coluna continuou 255 e o save dava **500 `TransactionSystemException: Could not commit JPA transaction`** (não é 400 de validação; o Spring loga só o topo, sem a causa raiz — o MySQL rejeitava no commit). Fix: 2º changeset `ong-descricao-varchar-1000-forcar` com **SQL cru** `ALTER TABLE ong MODIFY descricao VARCHAR(1000)` (dbms: mysql, sem precondição) — confirmado no log "ran successfully". **Lição: evitar precondição frágil com onError:MARK_RAN em modifyDataType; preferir ALTER direto.**
